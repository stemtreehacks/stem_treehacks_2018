...
function LineGraph(argsMap) {
	...
	var processDataMap = function(dataMap) {
		...
		var names = getRequiredVar(dataMap, 'names', "The data object must contain a 'names' value with a string array.")
        ...
        function max(tf) {
            var m = 0
            tf.forEach((x) => m = Math.max(m, x))
            return m;
        }

		dataValues.forEach(function (v, i) {
			newDataValues[i] = v.slice(0);
			maxValues[i] = max(newDataValues[i].map((x) => x[1]))
		})
	...
	var initX = function() {
        ...
        xmin = d3.min(data.values[0].map((x) => x[0]))
        xmax = d3.max(data.values[0].map((x) => x[0]))

		x = d3.time.scale().domain([xmin, xmax]).range([0, w]);
